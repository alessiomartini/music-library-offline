...
- `songs/<slug>.json` — the one place a song's non-audio identity lives:
  title, composer/lyricist, key, tempo, time signature, melody source
  (curated MusicXML path, or Basic Pitch with its filter thresholds), and
  the lyrics (syllabified, per verse). Every pipeline script reads this
  instead of hardcoding song-specific values.
- `input/<slug>.mp3` — the source recording.
- `working/<slug>/` — intermediate files: separated stems, vocal/harmony
  event JSON, aligned lyrics, assembled MusicXML. Fully regenerated by the
  pipeline; not hand-edited except curation notes like `curation-plan.md` /
  `musicxml-audit.md`.
